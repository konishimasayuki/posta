...
■ 表示時間との兼ね合い（重要）
  アニメーションは約1秒かかります。表示時間が短いテロップに
  派手な動き（letter系・roll系）を使うと、動き終わる前に消えてしまいます。

  表示時間ごとの選び方：
  - 1.5秒以上 … どの動きでもよい。letter系の派手なものも映える
  - 1.0〜1.5秒 … slide・scale・reveal など、素早く決まる動きにする
  - 1.0秒未満 … appear・reveal など、最も軽い動きだけにする。
                letter系・roll系・flying系は絶対に使わないこと

  テロップの個数が多い（6個以上）ときは、1個あたりの表示が
  1秒を切ることがあります。その場合は全体的に軽い動きで統一してください。
...
- 個数の目安（あくまで目安。ユーザーの指定がある場合はそちらが優先）
  ...
  1つのテロップにはアニメーション約1秒＋読む時間が必要なので、
  指定が無いときは詰め込みすぎないこと。
...
【役割の割り振りで注意すること】
同じ役割は上限を超えないよう、意識して振り分けること。
例：テロップが6個なら「hook1 + punch2 + info3」のように分散させる。
info ばかり5個にすると、5個目が捨てられてしまう。
